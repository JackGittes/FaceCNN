...
def FaceNet():
	...
	##Convolution layer 4
	kernel_4 = variable_with_weight_loss([1,1,64,32], stddev=0.01, wl=0.0,name='kernel_4')
	bias_4 = tf.Variable(tf.constant(0.01,shape=[32]),name='bias_4')
	relu4 = tf.nn.relu(conv2d(pool3,kernel_4)+bias_4)
	pool4 = tf.nn.max_pool(relu4,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')

	##Full-connected layer 1
	W_fc1 = variable_with_weight_loss([5*5*32,256], stddev=0.01, wl=0.003,name='W_fc1')
	b_fc1 = tf.Variable(tf.constant(0.01,shape=[256]),name='b_fc1')
	pool3_flat = tf.reshape(pool4,[-1,5*5*32])
